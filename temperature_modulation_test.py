...
def spon_spiking_somatic_temp_mod_paramjitter(P, dt, T, temp = np.asarray([34.0,34.0,34.0,34.0]), if_plot = 1, if_save = 1, base_dir = os.path.join(wd,'results'), save_dir = 'temp_mod_somatic'):
    rand_norm = norm
    rand_norm.random_state=RandomState(seed=None)
    Zm = rand_norm.rvs(0,1, size = 4)
    P['dist'][2:] = P['dist'][2:] + 0.05*P['dist'][2:]*Zm[:2] # jitter distance between the proximal/distal apical dendrite to soma by +/- 5% (SD)
    ...
    for t in temp:
        ...
        if if_save:
            ...
            burstiness = np.sum(isi_hist[0][:5])/len(isi)
            data_temp = {'v':v, 'P': cell.P,'gates':soln[1], 'T': T, 'dt': dt, 'burstiness': burstiness, 't_spike':t_spike}
            sio.savemat(os.path.join(datapath, 'temp%d_%s_%s.mat'%(cell.P["temp"][0],current_date, current_time)), data_temp)
dt = 0.05
T = 5000
for i in range(20):
    temp = np.asarray([34.0,28.0])
    # spon_spiking_temp_mod_paramjitter(P, dt, T, temp, if_plot = 0)
    spon_spiking_somatic_temp_mod_paramjitter(P, dt, T, temp, if_plot = 0)
